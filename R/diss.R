...
plotsmoothspec <- function ( lambdasX, YksX, lambdasY, YksY, myf, hX, hY , method="Maximum Likelihood") {
  ...
  if (pmatch(method , c("Maximum Likelihood", "Least Squares")) == 2) {
    specX <- exp(myf(YksX, lambdasX,  baseX, hX))
  } else {
  ...
  specY <- NULL
  if (pmatch(method , c("Maximum Likelihood", "Least Squares")) == 2) {
   	specY <- exp(myf(YksY,lambdasY, baseY, hY))
  } else {
...
distance.W.LS <- function(x, y, alpha, plot=FALSE) {
  ...
  integrateaux <- function( lambda ) {
    xx <- exp(myf(YksX, lambdasX, lambda, hX))
    yy <- exp(myf(YksY, lambdasY, lambda, hY))
    simetricDivergenceW(  xx / yy, alpha)
  }
...
diss.SPEC.LLR <- function(x,y, alpha=0.5, method="LS", plot=FALSE) {
  .check.equal.length.ts(x,y)
  typedist = 0
  type <-  (pmatch(method, c("LS", "LK" )))
  if (is.na(type)) {
     stop(paste("Unknown method", method))
  } else if (type == 1) {
    typedist <- distance.W.LS(x,y, alpha, plot)
  }
  else if (type == 2) {
...
pvalues.clust <- function(pvalues,significance) {
  ...
  is_pvalue_of_element_greater_than_significance_with_the_set <- function( element, setid, setlist, significance,distances) {
      ...
      if (plandist[i] < significacion) { #p-value < significance = YES
        ...
        if (conj < 2) {  #is each(ALL) series already in a cluster = NO
          ...
          if (conj > 0) { #one of the series in the pair already in a cluster = YES (x)
            if (is_pvalue_of_element_greater_than_significance_with_the_set( combinaciones[2,i], conj, grupos, significacion, distancias  )    ) {
              grupos <- create_new_set(combinaciones[2,i], grupos)
            } else {
            ...
            if (conj > 0) {#one of the series in the pair already in a cluster = YES (y)
              if (is_pvalue_of_element_greater_than_significance_with_the_set( combinaciones[1,i], conj, grupos, significacion, distancias  )    ) {
                grupos <- create_new_set(combinaciones[1,i], grupos)
              } else {
